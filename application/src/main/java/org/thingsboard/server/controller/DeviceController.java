...
public class DeviceController extends BaseController {
    ...
    @ApiOperation(value = "Get Device Id (getAllIdByDeviceType)",
            notes = "Fetch the Device Id available for the given device type. " +
                    DEVICE_TYPES_DESCRIPTION)
    @PreAuthorize("hasAnyAuthority('TENANT_ADMIN', 'CUSTOMER_USER')")
    @RequestMapping(value = "/device/allId/{deviceType}", method = RequestMethod.GET)
    @ResponseBody
    public List<String> getAllIdByDeviceType(@ApiParam(value = DEVICE_TYPE_PARAM_DESCRIPTION)
                                        @PathVariable(DEVICE_PROFILE_TYPE) String deviceType) throws ThingsboardException {
        return findAllIdByDeviceType(deviceType);
    }

...
import org.thingsboard.server.service.self_registration.SelfRegistrationService;
...
public class AdminController extends BaseController {
    ...
    @ApiOperation(value = "Get the SelfRegistration Settings (saveSelfRegistrationSettings) object",
            notes = "Get the Self-Registration Settings object that contains registration domain, etc." + SYSTEM_AUTHORITY_PARAGRAPH)
    @PreAuthorize("hasAuthority('TENANT_ADMIN')")
    @RequestMapping(value = "/selfRegistrationSettings", method = RequestMethod.GET)
    @ResponseBody
    public RegistrationSettings getSelfRegistrationSettings() throws ThingsboardException {
        accessControlService.checkPermission(getCurrentUser(), Resource.ADMIN_SETTINGS, Operation.READ);
        return checkNotNull(selfRegistrationService.getRegistrationSettings());
    }

    @ApiOperation(value = "Add/Update SelfRegistration Settings (saveSelfRegistrationSettings)",
            notes = "Add/Updates the Self-Registration Settings object that contains registration domain, etc." + TENANT_AUTHORITY_PARAGRAPH,
            produces = MediaType.APPLICATION_JSON_VALUE)
    @PreAuthorize("hasAuthority('TENANT_ADMIN')")
    @RequestMapping(value = "/selfRegistrationSettings", method = RequestMethod.POST)
    @ResponseBody
    public RegistrationSettings saveSelfRegistrationSettings(
            @ApiParam(value = "A JSON value representing the Self-Registration Settings.")
            @RequestBody RegistrationSettings registrationSettings) throws ThingsboardException {
        accessControlService.checkPermission(getCurrentUser(), Resource.ADMIN_SETTINGS, Operation.WRITE);
        registrationSettings = checkNotNull(selfRegistrationService.saveRegistrationSettings(registrationSettings));
        return registrationSettings;
    }

    @ApiOperation(value = "Delete selfRegistration Settings (saveSelfRegistrationSettings)",
            notes = "Deletes the selfRegistration settings."
                    + TENANT_AUTHORITY_PARAGRAPH)
    @PreAuthorize("hasAuthority('TENANT_ADMIN')")
    @RequestMapping(value = "/selfRegistrationSettings", method = RequestMethod.DELETE)
    @ResponseStatus(value = HttpStatus.OK)
    public void deleteSelfRegistrationSettings() throws ThingsboardException {
        accessControlService.checkPermission(getCurrentUser(), Resource.ADMIN_SETTINGS, Operation.DELETE);
        selfRegistrationService.deleteRegistrationSettings(TenantId.SYS_TENANT_ID);
    }

...
public class CustomerUserPermissions extends AbstractPermissions {
    ...
    public CustomerUserPermissions() {
        ...
        put(Resource.REPORT, customerReportPermissionChecker);
    ...
    private static final PermissionChecker customerReportPermissionChecker =
            new PermissionChecker.GenericPermissionChecker<ReportId, Report>(Operation.READ, Operation.READ_ATTRIBUTES, Operation.READ_TELEMETRY) {
                @Override
                public boolean hasPermission(SecurityUser user, Operation operation, ReportId reportId, Report report) {

                    if (!super.hasPermission(user, operation, reportId, report)) {
                        return false;
                    }
                    if (!user.getTenantId().equals(report.getTenantId())) {
                        return false;
                    }
                    return report.isAssignedToCustomer(user.getCustomerId());
                }
            };

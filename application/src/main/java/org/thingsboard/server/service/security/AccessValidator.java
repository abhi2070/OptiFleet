...
public class AccessValidator {
    ...
    public static final String DRIVER_WITH_REQUESTED_ID_NOT_FOUND = "Driver with requested id wasn't found!";
    ...
    public DeferredResult<ResponseEntity> validateEntityAndCallback(SecurityUser currentUser, Operation operation, String entityType, String entityIdStr, ThreeConsumer<DeferredResult<ResponseEntity>, TenantId, EntityId> onSuccess, BiConsumer<DeferredResult<ResponseEntity>, Throwable> onFailure) throws ThingsboardException {
        return validateEntityAndCallback(currentUser, operation, EntityIdFactory.getByTypeAndId(entityType, entityIdStr), onSuccess, onFailure);
    }
    ...
    public void validate(SecurityUser currentUser, Operation operation, EntityId entityId, FutureCallback<ValidationResult> callback) {
        switch (entityId.getEntityType()) {
            ...
            case ASSET_PROFILE:
                ...
                return;
            case SCHEDULER:
                validateScheduler(currentUser, operation, entityId, callback);
                return;
            case INTEGRATION:
                validateIntegration(currentUser, operation, entityId, callback);
                return;
            case ROLES:
                validateRoles(currentUser, operation, entityId, callback);
                return;
            case VEHICLE:
                validateVehicle(currentUser, operation, entityId, callback);
                return;
            case RULE_CHAIN:
            ...
            case ENTITY_VIEW:
                ...
                return;
            case DATACONVERTER:
                validateDataConverter(currentUser, operation, entityId, callback);
                return;
            case REPORT:
                validateReport(currentUser, operation, entityId, callback);
                return;
            case DRIVER:
                validateDriver(currentUser, operation, entityId, callback);
                return;
            case EDGE:
    ...
    private void validateDriver(final SecurityUser currentUser, Operation operation, EntityId entityId, FutureCallback<ValidationResult> callback) {
        if (currentUser.isSystemAdmin()) {
            callback.onSuccess(ValidationResult.accessDenied(SYSTEM_ADMINISTRATOR_IS_NOT_ALLOWED_TO_PERFORM_THIS_OPERATION));
        } else {
            Futures.addCallback(Futures.immediateFuture(driverService.findDriverById(currentUser.getTenantId(), new DriverId(entityId.getId()))), getCallback(callback, driver -> {
                if (driver == null) {
                    return ValidationResult.entityNotFound(DRIVER_WITH_REQUESTED_ID_NOT_FOUND);
                } else {
                    try {
                        accessControlService.checkPermission(currentUser, Resource.DRIVER, operation, entityId, driver);
                    } catch (ThingsboardException e) {
                        return ValidationResult.accessDenied(e.getMessage());
                    }
                    return ValidationResult.ok(driver);
                }
            }), executor);
        }
    }
    ...
    private void validateReport(final SecurityUser currentUser, Operation operation, EntityId entityId, FutureCallback<ValidationResult> callback) {
        if (currentUser.isSystemAdmin()) {
            callback.onSuccess(ValidationResult.accessDenied(SYSTEM_ADMINISTRATOR_IS_NOT_ALLOWED_TO_PERFORM_THIS_OPERATION));
        } else {
            ListenableFuture<Report> reportFuture = reportService.findReportByIdAsync(currentUser.getTenantId(), new ReportId(entityId.getId()));
            Futures.addCallback(reportFuture, getCallback(callback, report -> {
                if (report == null) {
                    return ValidationResult.entityNotFound("Report with requested id wasn't found!");
                } else {
                    try {
                        accessControlService.checkPermission(currentUser, Resource.REPORT, operation, entityId, report);
                    } catch (ThingsboardException e) {
                        return ValidationResult.accessDenied(e.getMessage());
                    }
    ...
    private void validateScheduler(final SecurityUser currentUser, Operation operation, EntityId entityId, FutureCallback<ValidationResult> callback) {
        if (currentUser.isSystemAdmin()) {
            callback.onSuccess(ValidationResult.accessDenied(SYSTEM_ADMINISTRATOR_IS_NOT_ALLOWED_TO_PERFORM_THIS_OPERATION));
        } else {
            ListenableFuture<Scheduler> schedulerFuture = schedulerService.findSchedulerByIdAsync(currentUser.getTenantId(), new SchedulerId(entityId.getId()));
            Futures.addCallback(schedulerFuture, getCallback(callback, scheduler -> {
                if (scheduler == null) {
                    return ValidationResult.entityNotFound("Scheduler with requested id wasn't found!");
                } else {
                    try {
                        accessControlService.checkPermission(currentUser, Resource.SCHEDULER, operation, entityId, scheduler);
                    } catch (ThingsboardException e) {
                        return ValidationResult.accessDenied(e.getMessage());
                    }
                    return ValidationResult.ok(scheduler);
                }
            }), executor);
        }
    }

    private void validateDataConverter(final SecurityUser currentUser, Operation operation, EntityId entityId, FutureCallback<ValidationResult> callback) {
        if (currentUser.isSystemAdmin()) {
            callback.onSuccess(ValidationResult.accessDenied(SYSTEM_ADMINISTRATOR_IS_NOT_ALLOWED_TO_PERFORM_THIS_OPERATION));
        } else {
            ListenableFuture<DataConverter> dataConverterFuture = dataConverterService.findDataConverterByIdAsync(currentUser.getTenantId(), new DataConverterId(entityId.getId()));
            Futures.addCallback(dataConverterFuture, getCallback(callback, dataConverter -> {
                if (dataConverter == null) {
                    return ValidationResult.entityNotFound("Data Converter with requested id wasn't found!");
                } else {
                    try {
                        accessControlService.checkPermission(currentUser, Resource.DATACONVERTER, operation, entityId, dataConverter);
                    } catch (ThingsboardException e) {
                        return ValidationResult.accessDenied(e.getMessage());
                    }
                    return ValidationResult.ok(dataConverter);
                }
            }), executor);
        }
    }
    ...
    private void validateRoles(final SecurityUser currentUser, Operation operation, EntityId entityId, FutureCallback<ValidationResult> callback) {
        if (currentUser.isSystemAdmin()) {
            callback.onSuccess(ValidationResult.accessDenied(SYSTEM_ADMINISTRATOR_IS_NOT_ALLOWED_TO_PERFORM_THIS_OPERATION));
        } else {
            ListenableFuture<Roles> rolesFuture = rolesService.findRolesByIdAsync(currentUser.getTenantId(), new RolesId(entityId.getId()));
            Futures.addCallback(rolesFuture, getCallback(callback, roles -> {
                if (roles == null) {
                    return ValidationResult.entityNotFound("Role with requested id wasn't found!");
                } else {
                    try {
                        accessControlService.checkPermission(currentUser, Resource.ROLES, operation, entityId, roles);
                    } catch (ThingsboardException e) {
                        return ValidationResult.accessDenied(e.getMessage());
                    }
                    return ValidationResult.ok(roles);
                }
            }), executor);
        }
    }

    private void validateVehicle(final SecurityUser currentUser, Operation operation, EntityId entityId, FutureCallback<ValidationResult> callback) {
        if (currentUser.isSystemAdmin()) {
            callback.onSuccess(ValidationResult.accessDenied(SYSTEM_ADMINISTRATOR_IS_NOT_ALLOWED_TO_PERFORM_THIS_OPERATION));
        } else {
            Futures.addCallback(Futures.immediateFuture(vehicleService.findVehicleById(currentUser.getTenantId(), new VehicleId(entityId.getId()))),
                    getCallback(callback, vehicle -> {
                        if (vehicle == null) {
                            return ValidationResult.entityNotFound("Vehicle with requested ID wasn't found!");
                        } else {
                            try {
                                accessControlService.checkPermission(currentUser, Resource.VEHICLE, operation, entityId, vehicle);
                            } catch (ThingsboardException e) {
                                return ValidationResult.accessDenied(e.getMessage());
                            }
                            return ValidationResult.ok(vehicle);
                        }
                    }), executor);
        }
    }

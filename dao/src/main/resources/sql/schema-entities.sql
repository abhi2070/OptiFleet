...
CREATE TABLE IF NOT EXISTS report (
    ...
    created_time bigint NOT NULL,
    tenant_id uuid,
    assigned_customers varchar,
    name varchar(255),
    type varchar(255),
    description varchar(1000),
    devices varchar,
    telemetry_keys varchar,
    external_id uuid,
...
CREATE TABLE IF NOT EXISTS driver (
    ...
    driving_license_validity bigint NOT NULL,
    profile_photo bytea,
    driving_license_doc bytea,
    CONSTRAINT fk_tenant FOREIGN KEY (tenant_id) REFERENCES tenant (id),
    CONSTRAINT unique_driving_license_number UNIQUE (driving_license_number)
);
...
CREATE TABLE IF NOT EXISTS data_converter (
    id uuid NOT NULL CONSTRAINT data_converter_pkey PRIMARY KEY,
    created_time bigint NOT NULL,
    additional_info varchar,
    configuration varchar,
    name varchar(255),
    tenant_id uuid,
    type varchar(255),
    script_lang varchar(255),
    payload varchar(100000),
    external_id uuid,
    CONSTRAINT data_converter_name_unq_key UNIQUE (tenant_id, name),
    CONSTRAINT data_converter_external_id_unq_key UNIQUE (tenant_id, external_id)
);
...
CREATE TABLE IF NOT EXISTS roles (
    ...
    customer_id uuid,
    external_id uuid,
    CONSTRAINT roles_name_unq_key UNIQUE (tenant_id, name),
    CONSTRAINT roles_external_id_unq_key UNIQUE (tenant_id, external_id)
);

CREATE TABLE IF NOT EXISTS vehicle_store (
    id uuid NOT NULL CONSTRAINT vehicle_pkey PRIMARY KEY,
    created_time bigint NOT NULL,
    tenant_id uuid NOT NULL,
    ...
    next_service bigint,
    status varchar(255),
    registration_certificate bytea,
    insurance_certificate bytea,
    puc_certificate bytea,
    required_permits bytea,
    CONSTRAINT vehicle_number_tenant_unq UNIQUE (tenant_id, vehicle_number)
);

CREATE TABLE IF NOT EXISTS vehicle_devices (
    vehicle_id uuid NOT NULL,
    device_id uuid,
    CONSTRAINT fk_vehicle FOREIGN KEY (vehicle_id) REFERENCES vehicle_store(id) ON DELETE CASCADE,
    CONSTRAINT fk_device FOREIGN KEY (device_id) REFERENCES device(id),
    CONSTRAINT vehicle_device_unq UNIQUE (vehicle_id, device_id)
);

CREATE TABLE IF NOT EXISTS scheduler (
    id uuid NOT NULL CONSTRAINT scheduler_pkey PRIMARY KEY,
    created_time bigint NOT NULL,
    bcc varchar,
    body varchar,
    cc varchar,
    ...
    repeatSchedule varchar(255),
    active boolean DEFAULT true,
    reportCategory varchar(255),
    reportAvailable varchar(255),
    external_id uuid,
    CONSTRAINT scheduler_name_unq_key UNIQUE (tenant_id, name),
    CONSTRAINT scheduler_external_id_unq_key UNIQUE (tenant_id, external_id)
);
...
CREATE TABLE IF NOT EXISTS integration (
    ...
    debug_mode BOOLEAN,
    allow_create_device_or_assets BOOLEAN,
    uplinkName varchar(255),
    Uplink_data_converter_name varchar(255),
    uplink_debugMode BOOLEAN,
    downlinkName varchar(255),
    Downlink_data_converter_name varchar(255),
    downlink_debugMode BOOLEAN,
    host varchar(255),
    port varchar(255),
    enableSSL BOOLEAN,
    password varchar(255),
    username varchar(255),
    topicFilters varchar(255),
    external_id uuid,
